...
if(!empty($_FILES['audio']['tmp_name'])){
    ...
    while(true){
        ...
        if(is_file('./resources/audio/'.$filename))
        $i++;
        else
        break;
    }
    ...
    if(strpos($type,"audio/") > -1 || strpos($type2,"audio/") > -1){
        $move = move_uploaded_file($file,'./resources/audio/'.$filename);
        if(!$move){
            $resp['status'] = 'failed';
            $resp['msg'] = 'Audio file updload failed';
            $resp['error'] = $conn->error;
        }else{
            $data .= ", `audio_path` = CONCAT('./resources/audio/{$filename}?v=',unix_timestamp(CURRENT_TIMESTAMP))";
        }
    ...
    if($save){
        ...
        if(!empty($_FILES['img']['tmp_name'])){
            ...
            while(true){
                if($i > -1)
                $filename = $i."_".$filename;
                if(is_file('./resources/images/'.$filename))
                $i++;
                else
                break;
            }
            if(strpos($type,"image/") > -1){
                $move = move_uploaded_file($file,"./resources/images/".$filename);
                if($move){
                    $conn->query("UPDATE `music_list` set image_path = CONCAT('./resources/images/{$filename}?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '{$mid}'");
                }else{
